class AppointmentAdminModel {
  final String id;
  final AppointmentStudentModel student;
  final String area;
  ...
  factory AppointmentAdminModel.fromJson(Map<String, dynamic> json) {
    final studentJson = json['student'] is Map
        ? Map<String, dynamic>.from(json['student'] as Map)
        : <String, dynamic>{};
    return AppointmentAdminModel(
      id: _readString(json, ['id']),
      student: AppointmentStudentModel.fromJson(studentJson),
      ...
      preferredTimeBlock:
          _readString(json, ['preferred_time_block', 'preferredTimeBlock']),
      scheduledStart: _readNullableString(
        json,
        ['scheduled_start', 'scheduledStart'],
      ),
      scheduledEnd:
          _readNullableString(json, ['scheduled_end', 'scheduledEnd']),
      status: _readString(json, ['status'], fallback: 'requested'),
      priority: _readString(json, ['priority'], fallback: 'normal'),
      assignedTo: _readNullableString(json, ['assigned_to', 'assignedTo']),
      createdAt: _readDate(json, ['created_at', 'createdAt']),
      updatedAt: _readDate(json, ['updated_at', 'updatedAt']),
      cancellationReason: _readNullableString(
          json, ['cancellation_reason', 'cancellationReason']),
      rescheduleReason:
          _readNullableString(json, ['reschedule_reason', 'rescheduleReason']),
      history: _readList(json['history'])
          .map((item) => AppointmentHistoryEntry.fromJson(item))
          .toList(),
    );
  }
...
class AppointmentStudentModel {
  ...
  factory AppointmentStudentModel.fromJson(Map<String, dynamic> json) {
    return AppointmentStudentModel(
      ...
      correoInstitucional:
          _readString(json, ['correo_institucional', 'correoInstitucional']),
